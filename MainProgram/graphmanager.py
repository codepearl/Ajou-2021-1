...
def GetCountplot(isArea, li, manager_name):                 # Countplot를 반환하기 위한 함수
    pure_data = dm.DataSearch(isArea, li, manager_name)                 # datamanager의 Datasearch 함수를 이용하여 필요한 데이터를 저장
    plt.rc('font', family='Malgun Gothic')                  # Font 설정
    return_countplot = Makecountplot(pure_data, isArea)                 # Makecountplot 함수를 사용하여 반환된 그래프를 저장
    return return_countplot


def GetPie(isArea, li, manager_name):                   # Pie graph를 반환하기 위한 함수
    pure_data = dm.DataSearch(isArea, li, manager_name)                 # datamanager의 Datasearch 함수를 이용하여 필요한 데이터를 저장
    plt.rc('font', family='Malgun Gothic')                  # Font 설정
    return_pie = MakePie(pure_data, isArea)                 # Makepie 함수를 사용하여 반환된 그래프를 저장
    return return_pie


def Makecountplot(pure_data, isArea):                 # Countplot을 생성하는 함수
    fig = plt.figure(figsize=(10, 6))                 # 캔버스 생성
    fig.set_facecolor('white')                  # 배경색 설정
    if isArea:                  # 입력된 데이터가 지역과 업종 중 지역이라면,
        made_countplot = sns.countplot(x='상권업종대분류명', data=pure_data)                  # x축을 업종대분류로 countplot 생성
    else:                  # 입력된 데이터가 지역과 업종 중 업종이라면,
        made_countplot = sns.countplot(x='법정동명', data=pure_data)                  # x축을 법정동명으로 countplot 생성

    plt.xticks(rotation=45)                 # 글자 겹침 문제를 해결하기위해 text를 45도 기울임
...
def MakePie(pure_data, isArea):
    ...
    for i, l in enumerate(labels):
        ...
        if frequency[i] / total * 100 < threshold:
            ...
            # x좌표가 양수이면, 즉 y축을 중심으로 오른쪽에 있으면 왼쪽 정렬 / 음수이면, 즉 y축을 중심으로 왼쪽에 있으면 오른쪽 정렬
...
def MakePieData(pure_data, isArea):                 # Pie graph 를 만들기 위한 data 를 생성하는 함수
    if isArea:                  # 입력데이터가 지역과 업종 중 지역이라면,
        ...
        list_filtered_data = pure_data['법정동명'].tolist()                 # 법정동명 데이터를 리스트로 저장
        set_filtered_data = set(list_filtered_data)                 # 중복 데이터 제거를 위해 set으로 변환
        list_filtered_data2 = list(set_filtered_data)
        count_list = []
        ratio_list = []
...
if __name__ == '__main__':
    return_graph = GetCountplot(True, ['세종특별자치시', '나성동'], "graphmanager")
